...
{
    [ExecuteInEditMode]
    public class FreeSimWorldBuilder : MonoBehaviour
    {
        #region Private Variables
        private static FreeSimWorldBuilder _lastActiveInstance;
        ...
        [SerializeField]
        private FreeSimScene _octave3DScene = new FreeSimScene();
        ...
        private FreeSimConfigSaveLoadSettings _configSaveSettings;
        private FreeSimConfigSaveLoadSettings _configLoadSettings;
        ...
        public FreeSimScene FreeSimScene { get { return _octave3DScene; } }
        public FreeSimConfigSaveLoadSettings ConfigSaveSettings
        {
            get
            {
                if (_configSaveSettings == null) _configSaveSettings = FreeSimWorldBuilder.ActiveInstance.CreateScriptableObject<FreeSimConfigSaveLoadSettings>();
                return _configSaveSettings;
            }
        }
        public FreeSimConfigSaveLoadSettings ConfigLoadSettings
        {
            get
            {
                if (_configLoadSettings == null) _configLoadSettings = FreeSimWorldBuilder.ActiveInstance.CreateScriptableObject<FreeSimConfigSaveLoadSettings>();
                return _configLoadSettings;
            }
        }
        public FreeSimConfigSaveWindow ConfigSaveWindow { get { return EditorWindowPool.FreeSimConfigSaveWindow; } }
        public FreeSimConfigLoadWindow ConfigLoadWindow { get { return EditorWindowPool.FreeSimConfigLoadWindow; } }
        ...
        public Inspector Inspector
        {
            get
            {
                if (_inspector == null)
                {
                    _inspector = FreeSimWorldBuilder.ActiveInstance.CreateScriptableObject<Inspector>();
        ...
        public static FreeSimWorldBuilder ActiveInstance 
        { 
            get 
            {
                ...
                FreeSimWorldBuilder octave3D = activeGameObject.GetComponent<FreeSimWorldBuilder>();
                if (octave3D != null)
                {
        ...
        public static int NumInstances
        {
            get
            {
                return FindObjectsOfType<FreeSimWorldBuilder>().Length;
            }
        }
        #endregion
        ...
        public List<GameObject> GetRoots(IEnumerable<GameObject> sceneObjects)
        {
            ...
            {
                var root = FreeSimWorldBuilder.ActiveInstance.GetRoot(gameObject);
        ...
        {
            return new List<Type> { typeof(FreeSimWorldBuilder), typeof(Terrain) };
        }
        ...
        [MenuItem("Tools/FreeSim/Deselect &D")]
        private static void DeselectObjectGrid3D()
        {
            if (FreeSimWorldBuilder.ActiveInstance != null)
            {
                FreeSimWorldBuilder.ActiveInstance.DeselectInSceneView();
        ...
        [MenuItem("Tools/FreeSim/Select &R")]
        private static void SelectObjectGrid3D()
        {
            FreeSimWorldBuilder[] instances = FindObjectsOfType<FreeSimWorldBuilder>();
            if (instances.Length != 0) instances[0].SelectInSceneView();
        }

        [MenuItem("Tools/FreeSim/Focus #&F")]
        private static void FocusGameObject()
        {
            if (FreeSimWorldBuilder.ActiveInstance != null)
            {
                var currentActive = Selection.activeGameObject;
                Selection.activeGameObject = FreeSimWorldBuilder.ActiveInstance.ObjectSelection.GetLastSelectedGameObject();
                if (SceneView.lastActiveSceneView != null) SceneView.lastActiveSceneView.FrameSelected();
                Selection.activeGameObject = currentActive;

                FreeSimWorldBuilder.ActiveInstance.SelectInSceneView();
            }
        }

        [MenuItem("GameObject/FreeSim/Make group", false, 0)]
        private static void MakeGroup()
        {
            FreeSimWorldBuilder octave3D = FreeSimWorldBuilder.ActiveInstance;
            if (octave3D == null)
            {
                Debug.LogWarning("There is no FreeSim object in the scene. Please create an empty game object and attach the FreeSim World Builder script to it.");
                return;
            }
        ...
        [MenuItem("GameObject/FreeSim/Is not group", false, 0)]
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (Selection.activeTransform.GetComponent<FreeSimWorldBuilder>() == null) return;
        ...
        {
            return FileSystem.GetToolFolderName() + "/Scriptable Objects/FreeSim_" + this.GetHashCode();
        }
        ...
        private void DeselectInSceneView()
        {
            ...
            selectedObjects.Remove(FreeSimWorldBuilder.ActiveInstance.gameObject);
            Selection.objects = selectedObjects.ToArray();
        }
