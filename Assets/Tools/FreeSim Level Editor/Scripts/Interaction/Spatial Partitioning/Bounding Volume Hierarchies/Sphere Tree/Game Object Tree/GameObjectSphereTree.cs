...
namespace FreeSimEditor
{
    ...
    public class GameObjectSphereTree : ISerializationCallbackReceiver
    {
        ...
        public bool ObjectMeshIntersectsAnyMesh(FreeSimMesh octave3DQueryMesh, TransformMatrix worldMatrix, HashSet<GameObject> ignoreObjects)
        {
            if (ignoreObjects == null) ignoreObjects = new HashSet<GameObject>();
            ...
            foreach (SphereTreeNode<GameObject> node in allOverlappedNodes)
            {
                ...
                if (objectMesh != null)
                {
                    FreeSimMesh octave3DMesh = FreeSimMeshDatabase.Get().GetFreeSimMesh(objectMesh);
        ...
        public List<GameObjectRayHit> RaycastAllMesh(Ray ray)
        {
            ...
            FreeSimMeshDatabase octave3DMeshDatabase = FreeSimMeshDatabase.Get();
            foreach(var boxHit in allBoxHits)
            {
                ...
                FreeSimMesh octave3DMesh = octave3DMeshDatabase.GetFreeSimMesh(objectMesh);
        ...
        public void RegisterGameObject(GameObject gameObject)
        {
            ...
            foreach (var light in lights) FreeSimScene.Get().RegisterSceneLight(light);
        ...
        public void HandleTransformChangesForAllRegisteredObjects()
        {
            ...
            if (FreeSimWorldBuilder.ActiveInstance != null && Selection.Contains(FreeSimWorldBuilder.ActiveInstance.gameObject))
            {
        ...
        public void Rebuild(bool showProgress)
        {
            ...
            List<GameObject> allWorkingObjects = FreeSimWorldBuilder.ActiveInstance.GetAllWorkingObjects();
            if(showProgress)
            {
        ...
        private bool CanGameObjectBeRegisteredWithTree(GameObject gameObject)
        {
            ...
            if (!FreeSimWorldBuilder.ActiveInstance.IsWorkingObject(gameObject)) return false;
            if (FreeSimWorldBuilder.ActiveInstance.IsPivotWorkingObject(gameObject)) return false;
        ...
        public void RegisterUnregisteredObjects()
        {
            ...
            {
                List<GameObject> workingObjects = FreeSimWorldBuilder.ActiveInstance.GetAllWorkingObjects();
