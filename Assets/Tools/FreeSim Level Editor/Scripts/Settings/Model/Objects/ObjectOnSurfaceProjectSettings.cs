...
namespace FreeSimEditor
{
    ...
    public class ObjectOnSurfaceProjectSettings : ScriptableObject
    {
        ...
        public GameObject ProjectionSurface
        {
            ...
            set
            {
                if (CanBeUsedAsProjectionSurface(value)) _projectionSurface = value;
                                      ...
                                      "and it must be either a terrain object with a terrain collider or mesh object. If it is a mesh object, it must be a child of FreeSim.");
            }
        }
        ...
        public bool CanBeUsedAsProjectionSurface(GameObject gameObject)
        {
            ...
            if (gameObject.HasMesh()) return FreeSimWorldBuilder.ActiveInstance.IsWorkingObject(gameObject);
        ...
        public void RenderView()
        {
            ...
            if(_isVisible)
            {
                ...
                if(!ProjectOnGrid)
                {
                    FreeSimWorldBuilder.ActiveInstance.ShowGUIHint("The surface projection object must be a terrain object (with a terrain collider) or mesh (no colliders required). " +
                                                                    "If it is a mesh, it must also be a child of FreeSim.");
                    content.text = "Surface object";
                    content.tooltip = "The object which acts as a projection surface. Must be a terrain (with a mesh collider attached) or mesh object. If it is a mesh object, it must be a child of FreeSim.";
