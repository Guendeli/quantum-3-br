...
namespace FreeSimEditor
{
    public static class GameObjectExtensions
    {
        ...
        public static FreeSimMesh GetFreeSimMesh(this GameObject gameObject)
        {
            Mesh mesh = gameObject.GetMeshFromFilterOrSkinnedMeshRenderer();
            if (mesh == null) return null;

            return FreeSimMeshDatabase.Get().GetFreeSimMesh(mesh);
        }
        ...
        public static List<Vector3> GetOverlappedVertsInHierarchy(this GameObject root, OrientedBox overlapBox)
        {
            ...
            foreach (var gameObject in allObjects)
            {
                ...
                if (mesh != null)
                {
                    FreeSimMesh octave3DMesh = FreeSimMeshDatabase.Get().GetFreeSimMesh(mesh);
        ...
        public static List<FreeSimMesh> GetAllFreeSimMeshesInHierarchy(this GameObject root)
        {
            List<GameObject> allObjects = root.GetAllChildrenIncludingSelf();
            if (allObjects.Count == 0) return new List<FreeSimMesh>();

            var allFreeSimMeshes = new List<FreeSimMesh>(allObjects.Count);
            foreach(var gameObject in allObjects)
            {
                Mesh mesh = gameObject.GetMeshFromFilterOrSkinnedMeshRenderer();
                if(mesh != null)
                {
                    FreeSimMesh octave3DMesh = FreeSimMeshDatabase.Get().GetFreeSimMesh(mesh);
                    if (octave3DMesh != null) allFreeSimMeshes.Add(octave3DMesh);
                }
            }

            return allFreeSimMeshes;
        }
        ...
        public static GameObject CloneAsWorkingObject(this GameObject gameObject, Transform parentTransform, bool allowUndoRedo = true)
        {
            ...
            FreeSimScene.Get().RegisterObjectHierarchy(clone);
        ...
        public static bool RaycastMesh(this GameObject gameObject, Ray ray, out GameObjectRayHit objectRayHit)
        {
            ...
            FreeSimMesh octaveMesh = FreeSimMeshDatabase.Get().GetFreeSimMesh(objectMesh);
        ...
        public static Box GetHierarchyModelSpaceBox(this GameObject hierarchyRoot)
        {
            ...
            foreach (Transform childTransform in allChildTransforms)
            {
                ...
                {
                    /*FreeSimMesh octave3DMesh = FreeSimMeshDatabase.Get().GetFreeSimMesh(childObject.GetMeshFromFilterOrSkinnedMeshRenderer());
        ...
        public static Box GetNonMeshModelSpaceBox(this GameObject gameObject)
        {
            ...
            return new Box(Vector3.zero, FreeSimScene.VolumeSizeForNonMeshObjects);
        }
