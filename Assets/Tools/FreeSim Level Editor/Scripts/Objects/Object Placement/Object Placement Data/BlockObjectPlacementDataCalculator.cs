            ...
            for (int segmentIndex = 0; segmentIndex < _block.NumberOfSegments; ++segmentIndex)
            {
                ...
                for (int stackIndex = 0; stackIndex < segment.NumberOfStacks; ++stackIndex)
                {
                    ...
                    if (canProject)
                    {
                        ...
                        if (projectionSettings.CanProjectOnMesh) closestMeshHit = FreeSimScene.Get().RaycastAllMeshClosest(ray);
                        if (projectionSettings.CanProjectOnTerrain) closestTerrainHit = FreeSimScene.Get().RaycastAllTerainsClosest(ray);
