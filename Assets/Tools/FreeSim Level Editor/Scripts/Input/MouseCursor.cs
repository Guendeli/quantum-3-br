...
namespace FreeSimEditor
{
    public class MouseCursor : Singleton<MouseCursor>
    {
        ...
        private void RaycastAllObjectsNoTerrains(Ray ray, List<GameObjectRayHit> objectHits)
        {
            ...
            {
                List<GameObjectRayHit> objectMeshHits = FreeSimScene.Get().RaycastAllMesh(ray);
                if (objectMeshHits.Count != 0) objectHits.AddRange(objectMeshHits);

                List<GameObjectRayHit> objectBoxHits = FreeSimScene.Get().RaycastAllBox(ray);
                ...
                List<GameObjectRayHit> objectSpriteHits = FreeSimScene.Get().RaycastAllSprite(ray);
